...
> * **1.IVERILOG:** _Icarus Verilog (often abbreviated as IVerilog) is an open-source hardware description language (HDL) simulator.It compiles the Verilog/SystemVerilog code and produces a simulation executable that simulates the behavior of the described hardware._
...
> * **VERILOG CODE:-** _The code implements a pipelined RV32I processor with basic arithmetic, logic, load/store, and branch instructions.
It supports instructions like ADD, SUB, AND, OR, XOR, SLT, ADDI, SUBI, ANDI, ORI, XORI, LW, SW, BEQ, BNE, SLL, and SRL.
The processor consists of the IF, ID, EX, MEM, and WB pipeline stages.
It fetches instructions from memory, decodes them, executes them, accesses memory if necessary, and writes back the results.
